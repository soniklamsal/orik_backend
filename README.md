...
67 tests covering enquiry validation, the honeypot, the rate limit, proxy IP
...
The repo carries a Blueprint, so Render can create the web service and its
...
2. Before the first deploy, set these in the service's **Environment** tab
   (they are marked `sync: false`, so Render asks rather than guessing):
   ...
   | `CORS_ALLOWED_ORIGINS` | `https://orikwebcraft.com,https://www.orikwebcraft.com` |
   | `SITE_URL` | `https://orikwebcraft.com` |

   `DJANGO_SECRET_KEY` is generated by Render, `DATABASE_URL` is wired from the
   database, and `DJANGO_DEBUG` is already `False`.
3. Deploy. `build.sh` installs dependencies, runs `collectstatic`, applies
   migrations, seeds the starting content and creates the admin user.
4. Point the frontend at it: set `NEXT_PUBLIC_API_URL` in Vercel to
   `https://<your-service>.onrender.com` and redeploy.

### How the pieces fit

- **Database** — `DATABASE_URL` wins if set, then the `POSTGRES_*` vars, then
  SQLite. TLS is required whenever `DEBUG` is off, and connections are reused
  for 10 minutes.
- **Static files** — WhiteNoise serves them, compressed and fingerprinted, so
  the admin and Jazzmin are styled without a separate CDN.
- **Allowed hosts** — Render's `RENDER_EXTERNAL_HOSTNAME` is appended
  automatically, so the service works before you attach a custom domain.
- **Health check** — Render polls `/api/health/`.

### Two limits of the free plan

- **The service sleeps after inactivity.** The first request afterwards takes
  ~30 seconds. The frontend renders its bundled copy rather than hanging, so a
  cold start shows slightly stale content instead of an error.
- **Uploaded images do not survive a deploy.** Render's disk is ephemeral, so
  team photos uploaded in the admin are lost on the next build. Attach a Render
  disk mounted at `MEDIA_ROOT`, or move to S3/Cloudinary, before relying on
  them. Everything else is in Postgres and unaffected.
...
4. Set `DATABASE_URL` (or the `POSTGRES_*` variables).
5. Run `manage.py collectstatic`; WhiteNoise serves the result.
